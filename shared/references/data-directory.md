...
All user data lives in a `.jobs-agent/` folder. Follow these steps to find it:

## Resolution Algorithm

1. Check the current working directory for `.jobs-agent/` — use it if found
2. Check `~/.jobs-agent/` — use it if found
3. If neither exists:
   - **setup skill**: this is a fresh setup — create it in Step 1
   - **all other skills**: tell the user to run `/jobs-agent:setup` first, then stop
...
All paths in skill instructions use `DATA_DIR` to mean whichever `.jobs-agent/` directory was found or created.
